...
include("e:/richard_dennis/econ5086/julia_functions/newton.jl")
include("e:/richard_dennis/econ5086/julia_functions/derivative.jl")
...
function stoch_growth_model(x::Array{Float64,1})
...
while len > 1e-5
  ...
  for i = 1:size(grid,1)
    ...
    soln,f_soln,iters = newton(stoch_growth_model,initial,tol,maxiters)
    new_capital[i]     = soln[1]
    new_consumption[i] = soln[2]

  end

  len = maxabs([(new_consumption-consumption);(new_capital-capital)])
  println(len)
